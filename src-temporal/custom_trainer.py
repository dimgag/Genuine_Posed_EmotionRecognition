...
class BaseTrainer:
    ...
    def train_metrics(self):
        metrics = {
            "acc": self.last_train_result["acc"],
            "loss": self.last_train_result["loss"],
        }
        return metrics
    ...
    def val_metrics(self):
        metrics = {
            "val_acc": self.last_val_result["acc"],
            "val_loss": self.last_val_result["loss"],
        }
        return metrics
    ...
    def _one_val_loop(self, pbar, len_val_dataloader):
        ...
        for step in range(len_val_dataloader):
            ...
            # add accuracy
            val_acc += self.validation_step_acc(batch)
    ...
    def fit(self):
        ...
        for epoch in range(self.starting_epoch, self.hparams["max_epochs"]):
            ...
            with tqdm(
                ...
                # add accuracy
                train_acc = self._one_train_loop_acc(pbar, len_train_dataloader)
                ...
                # add accuracy
                train_acc = train_acc / len_train_dataloader

                train_score = self._get_last_train_score()
                pbar.set_postfix({"loss": f"{train_loss:.4f}", train_score[0]: f"{train_score[1]:.3f}", "acc": f"{train_acc:.3f}"})
                ...
                with torch.inference_mode():
                    ...
                    # add accuracy
                    val_acc = self._one_val_loop_acc(pbar, len_val_dataloader)
                ...
                # add accuracy
                val_acc = val_acc / len_val_dataloader
