...
def parse_video_frames():
    ...
    video_input_paths, video_output_dirs = video_parser(input_dir, output_dir)
    ...
    for video_input_file, video_output_dir in zip(video_input_paths, video_output_dirs):
        ...
        if os.path.isfile(video_input_file):
            ...
            while cap.isOpened():
                ...
                img = []
                if ret==True:
                    face_img = crop_face(frame, rotate=alignment, quiet_mode=quiet_mode)
                   
                    if face_img is not None:
                        face_img = face_img.resize((size, size))
                        face_img = PIL_image_convert(np.array(face_img))
                        print(f'frame {current_frame}/{total_frames} from video {filename} is processed')
                        out.write(np.array(face_img))
